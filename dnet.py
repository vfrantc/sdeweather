...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Training of the retinex splitting network')
    parser.add_argument('--epochs', dest='epochs', type=int, default=100, help='number of total epochs') # epochs to train for
    parser.add_argument('--batch_size', dest='batch_size', type=int, default=16, help='number of samples in one batch') # depends on your GPU memory
    parser.add_argument('--patch_size', dest='patch_size', type=int, default=96, help='patch size') # the patch size of the input image
    parser.add_argument('--lr', dest='lr', type=float, default=0.001, help='initial learning rate') #
    parser.add_argument('--data_dir', dest='data_dir', default='./data/', help='data directory') # the directory of the data
    ...
    if not os.path.exists(args.ckpt_dir): # create the dir for the checkpoints
        os.makedirs(args.ckpt_dir)
    if not os.path.exists(args.vis_dir): # create the dir for the visualizations
    ...
    # get
    train_low_data_names = glob(os.path.join(args.data_dir, 'LOLdataset/our485/low/*.png')) + glob(os.path.join(args.data_dir , 'BrighteningTrain/low/*.png'))
    train_low_data_names.sort()
    train_high_data_names = glob(args.data_dir + 'LOLdataset/our485/high/*.png') + glob(args.data_dir + 'BrighteningTrain/high/*.png')
    train_high_data_names.sort()
    eval_low_data_names = glob(args.data_dir + 'LOLdataset/eval15/low/*.*')
